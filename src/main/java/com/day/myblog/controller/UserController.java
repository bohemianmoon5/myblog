...
import com.day.myblog.dto.KakaoProfile;
import com.day.myblog.dto.OAuthToken;
import com.day.myblog.dto.User;
import com.day.myblog.dto.UserBuilder;
...
public class UserController {
	...
	public String kakaoCallback(String code) throws FindException {
		...
		params.add("client_id", "30c061b6dc1027d8348c07e998a3504c");
...
//		System.out.println("블로그서버 패스워드 : " + myblogKey);
		
		User kakaoUser = new User();
		kakaoUser.setUsername(kakaoProfile.getKakao_account().getEmail()+"_"+kakaoProfile.getId());
		kakaoUser.setPassword(myblogKey);
		kakaoUser.setEmail(kakaoProfile.getKakao_account().getEmail());
		kakaoUser.setOauth("kakao"); //kakao 로그인 사용자
		
		//가입자 or 비가입자 check 
		User originUser = userService.selectByName(kakaoUser.getUsername());
		...
		//로그인 처리 
		Authentication authentication = authenticationManager.authenticate(new UsernamePasswordAuthenticationToken(kakaoUser.getUsername(), myblogKey));
